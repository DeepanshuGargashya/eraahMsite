...
function Donation() {
    ...
    return (
        <>
            ...
            <div className="DonationPage" style={{ position: tabs === 'Donation' || tabs === 'Payment' ? 'relative' : '', height: tabs === 'Donation' ? '100%' : '' }}>
                <div className="top-section" style={{ position: 'sticky', top: '0' }}>
                    <div className="heading">
                        <h5><span className='closeicon' onClick={() => navigate(-1)}><ClearIcon /></span>&nbsp;&nbsp;&nbsp;&nbsp;
                ...
                <div className="top-section">
                    <div className="tabs d-flex text-center justify-content-center">
                        ...
                        <div className={`line ${tabs === 'Payment' || tabs === 'Review' ? '' : 'hide'}`}></div>
                        ...
                        <>
                            <div className="donationTabs">
                                ...
                                <div className="amount-circles">
                                    {
                                        amountcircle.map((value) => {
                                            return (
                                                <div className={`circle ${amount === value ? 'active' : ''}`} onClick={() => handleAmount(value)}>
                                                    <h5>{value}</h5>
                                                </div>
                                            )
                                        })
                                ...
                                <div class="input-group Amountinput mb-3">
                                    <input type="number" min="0" class="form-control inputtag" placeholder="Enter Other Amount" aria-label="search" aria-describedby="basic-addon1" value={Inputamount > 0 ? Inputamount : ''} onChange={(e) => setInputamount(e.target.value)} />
                                    <span class="input-group-text inputag-text" id="basic-addon1">
                                        INR
                                    </span>
                                </div>
                            </div>
                            <div className="bottombtns" style={{ position: 'absolute', width: '100%', bottom: '0' }}>
                                <div className="box">
                                    <div className="totalamount">

                                        <h5 className='LEfth5'>Total Donation</h5>
                                        <h5 className='righth5'><span>{Number(amount) + Number(Inputamount)}</span>&nbsp;&nbsp;INR</h5>
                                    </div>
                                    <div className="btns mt-3 text-center">
                                        <button className='btn' onClick={() => settabs('Payment')} disabled={(Number(amount) + Number(Inputamount)) === 0 ? true : false}>Proceed to Payment</button>
                                    </div>
                                </div>
                            </div>
                        </>
                                ...
                                <div className="radioss">
                                    {
                                        PaymentMethod.map((value, index) => {
                                            return (
                                                <div class={`form-check formgrp d-flex ${selectedPayment === value.name ? 'activeradio' : ''}`} key={index} onClick={() => handleRadioChange(value.name)}>
                                                    <input class="form-check-input radioinput" type="radio"
                                                        name="paymentMethod"
                                                    ...
                                                    />&nbsp;&nbsp;
                                                    <label class="form-check-label radiolabel" htmlFor={`flexRadioDefault${index}`}>
                                                        <img src={value.image} alt=" image" width={value.image === cash ? '13%' :
                                                            value.image === card ? '16%' : '20%'} />&nbsp;
                                                        {value.name}
                                                    </label>
                                                </div>
                                            )
                            ...
                            </div>
                            {
                                selectedPayment === 'UPI' ?
                                    ...
                                    <div className="background-blur"></div>
                                        <div className={`bottombtns ${selectedPayment === 'UPI' ? 'animate__animated animate__slideInUp' : ''}`} style={{ position: 'sticky', zIndex: '2', bottom: '0' }}>
                                            <div className="text-center">
                                                <img src={arrowDown} width={'10%'} alt="down-arrow"  onClick={()=>setSelectedPayment(null)} style={{cursor:'pointer'}}/>
                                            </div>
                                            <div className="box">
                                                <label className='headinglabel mt-2'>Enter your UPI ID</label>
                                                <div className="formgrp mt-1">
                                                    <input type="text" className='inputtag' placeholder='number@paytm' value={upiInput} onChange={(e) => setupiInput(e.target.value)} />
                                                    <button className='verifybtn' onClick={()=>navigate('/homePage')}>Verify</button>
                                                </div>
                                                ...
                                                <div className="otherways mt-1">
                                                    <div className="topways">
                                                        <h5>
                                                            <img src={paytm} width={"20%"} alt="image" />&nbsp;&nbsp;Paytm UPI
                                                        </h5>
                                                    ...
                                                    <div className="topways bottom">
                                                        <h5>
                                                            <img src={GooglePay} width={"15%"} alt="image" />&nbsp;&nbsp;Google Pay UPI
                                                        </h5>
                                                        <div className="square">
                                                            <h5>Link</h5>
                                                        </div>
